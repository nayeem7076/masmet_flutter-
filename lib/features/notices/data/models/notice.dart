class NoticeItem {
  ...
  factory NoticeItem.fromApi(Map<String, dynamic> json) {
    return NoticeItem(
      id: (json['id'] ?? json['notice_id'] ?? '').toString(),
      title: (json['title'] ?? json['subject'] ?? 'Notice').toString(),
      ...
      date: DateTime.tryParse(
          ...
          DateTime.now(),
      sender: (json['sender'] ?? json['author'] ?? 'Manager').toString(),
      sendToAll: (json['sendToAll'] ?? true) == true,
      targetMemberIds:
          List<String>.from((json['targetMemberIds'] ?? const <String>[])),
    );
  }
}
